 ...
 * - Auth timeout (5s)
 * - Indoor button
 * - Không time-multiplex (servo/buzzer, keypad/nút tách riêng)
 */
...
void setup() {
  ...
  delay(500);
  Serial.println("\n\n=== SMART DOOR - FULL VERSION ===");
  Serial.println("Board: ESP32 (CP2102)");
  ...
  // Keypad pin modes (ESP32 không có KEYMAP preset pull-up)
  pinMode(KEYPAD_ROW_0, OUTPUT);
  pinMode(KEYPAD_ROW_1, OUTPUT);
  pinMode(KEYPAD_ROW_2, OUTPUT);
  pinMode(KEYPAD_ROW_3, OUTPUT);
  pinMode(KEYPAD_COL_0, INPUT_PULLUP);
  pinMode(KEYPAD_COL_1, INPUT_PULLUP);
  pinMode(KEYPAD_COL_2, INPUT_PULLUP);
  pinMode(KEYPAD_COL_3, INPUT_PULLUP);

  pinMode(INDOOR_BUTTON_PIN, INPUT_PULLUP);
  pinMode(BUZZER_PIN, OUTPUT);
  digitalWrite(BUZZER_PIN, LOW);
